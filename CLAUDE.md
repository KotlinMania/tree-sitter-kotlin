# Claude Code Project Instructions — tree-sitter-kotlin

## Project Overview

This is **tree-sitter-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`tree-sitter`](https://crates.io/crates/tree-sitter).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/tree-sitter/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/tree-sitter>
